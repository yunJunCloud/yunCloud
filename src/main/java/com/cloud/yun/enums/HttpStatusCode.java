...
public enum HttpStatusCode implements YunCloudCode {
	...
	HttpStatusCode(int value, String enMessage, String zhMessage) {
		this.value = value;
	...
	public String getZhMessage(int value){
		List<HttpStatusCode> collect = Arrays.stream(HttpStatusCode.values()).filter(httpStatusCode -> httpStatusCode.value == value).collect(Collectors.toList());
		if(CollUtil.isNotEmpty(collect)){
			return collect.get(0).getZhMessage(value);
		}
		return "";
	}

	public String getEnMessage(int value){
		List<HttpStatusCode> collect = Arrays.stream(HttpStatusCode.values()).filter(httpStatusCode -> httpStatusCode.value == value).collect(Collectors.toList());
		if(CollUtil.isNotEmpty(collect)){
			return collect.get(0).getEnMessage(value);
		}
		return "";
	}
